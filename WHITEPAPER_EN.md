...
3. For each new transaction, it does one simple piece of maths:
   *"How much SOL did this wallet have before, and how much after?"*
   - More after than before → **money came in** → that's a fee being collected.
   - Same or less → nothing to report, stay quiet.
4. If money came in, it **sends you a Telegram message** with the exact amount
   and a link to the receipt (the transaction on Solscan).

There's no guessing and no insider access. It's just reading the public record
and doing subtraction.
...
- By default it treats **any incoming SOL** as "a fee." If a wallet receives SOL
  for some other reason, that still counts. (This can be tightened later to only
  count specific fee-claim transactions.)
